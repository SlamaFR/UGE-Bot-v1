...
public class TrafficNotifier
{
    ...
    private TrafficNotifier() {
        ...
        this.linesMeta = Stream.of(new Object[][]{{"RA", new String[]{"RER A", EmotesUtils.RER, EmotesUtils.RER_A}}, {"RE", new String[]{"RER E", EmotesUtils.RER, EmotesUtils.RER_E}}}).collect(Collectors.toMap(icons -> (String) icons[0], icons -> (String[]) icons[1]));
    ...
    private void refreshData() {
        try {
            ...
            for (String s : ratpObject.keySet()) {
                if (ratpObject.getJSONArray(s).length() == 0) {
                    continue;
                }
    ...
    private void checkAndSendMessage(String s, String message) {
        if (lastMessages.containsKey(s) && lastMessages.get(s).contains(message)) {
            return;
        }
        if (!lastMessages.containsKey(s)) {
            lastMessages.put(s, new ArrayList<>());
        }
        lastMessages.get(s).add(message);

        if (veryLastMessage.getOrDefault(s, "").equals(message)) {
            return;
        }
    ...
    private void sendNotification(String modeEmote, String lineEmote, String title, String message) {
        for (Long textChannelId : textChannelsIds) {
            ...
            textChannel.sendMessage(new EmbedBuilder().addField(String.format("%s %s %s", modeEmote, lineEmote, title), message, false).setFooter("Plus d'informations sur ratp.fr ou transilien.com").setTimestamp(Instant.now()).setColor(getColor(message)).build()).queue();
        }
    }

    private Color getColor(String message) {
        if (Pattern.matches(".*travaux.*", message)) {
            return new Color(0xe67e22);
        }
        if (Pattern.matches(".*(perturb|panne|interrompu|accident|sécurité).*", message)) {
